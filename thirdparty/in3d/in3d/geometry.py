...
class AxisList(LineGeometry):
    def __init__(
        self, program, length, thickness=1.0
    ):
        super().__init__()
        self.program = program
        self.lines = []
    ...
    @staticmethod
    def transform_vertices(Tpw, lines, thickness=1):
        v = np.dot(Tpw, lines.reshape(-1, 4).T).T.copy()
        v[:, -1] = thickness
        return v.reshape(-1)
    ...
    def add(self, Tpw):
        ...
        lines = np.array(
            [
                0, 0, 0, self.thickness, self.length, 0, 0, self.thickness,
                0, 0, 0, self.thickness, 0, self.length, 0, self.thickness,
                0, 0, 0, self.thickness, 0, 0, self.length, self.thickness,
            ], dtype=np.float32,
        )
        colors = np.array(
            [1, 0, 0, 1, 
        ...
        lines = AxisList.transform_vertices(Tpw, lines, self.thickness)
