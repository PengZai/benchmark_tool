...
from dense_slam_benchmark.benchmark_tools.utils import updateConfig, saveMetricsLogAndResults
...
def dense_n_view_benchmark(config):
    ...
    with torch.no_grad():
        ...
        for batch_iter_idx, frames in enumerate(test_dataloader):

            num_frame = len(frames)
            batch_size = frames[0][0]['undistorted_image'].shape[0]

            sample_idx_in_batch = [
                frames[0][0]['idx'][b].item() for b in range(batch_size)
            ]
            print(
                f"[{batch_iter_idx + 1}/{num_batches}] "
