...
import numpy as np
import json
from dense_slam_benchmark.dataset_tools.utils import invert_transform
from omegaconf import DictConfig, OmegaConf
from dense_slam_benchmark.dataset_tools.utils import depth2color, save_depth_histogram, depth_range_by_ratio
import cv2
...
def saveMetricsLogAndResults(config, result_list):
    ...
    output_path = os.path.join(config.machine.root_experiments_dir, f"dense_{config.num_view_for_sub_scene}_view_stride_{config.stride_for_sub_scene}", config.model.model_str)
    ...
    all_results = list(deduped_by_sample_idx.values())
